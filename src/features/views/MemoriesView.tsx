...
export function MemoriesView({ relationshipId }: { relationshipId: string }) {
  ...
  return (
    <div className="mx-auto max-w-md space-y-3 px-5 py-6 pb-32">
      ...
      {memories.map((m) => (
        <article key={m.id} className="overflow-hidden rounded-3xl border border-white/40 bg-white/50 backdrop-blur-xl">
          {m.cover_url && (
            <button onClick={() => setLightbox(m.cover_url)} className="block w-full">
              <img src={m.cover_url} alt="" loading="lazy" className="h-48 w-full object-cover" />
            </button>
          )}
          <div className="p-5">
