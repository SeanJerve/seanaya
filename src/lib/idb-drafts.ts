...
export function useDraft<T extends object>(key: string, state: T, setState: (v: T) => void, enabled = true) {
  const loaded = useRef(false);
  const [hydrated, setHydrated] = useState(false);
  useEffect(() => {
    if (!enabled || loaded.current) return;
    loaded.current = true;
    drafts.get<T>(key).then((v) => { if (v) setState({ ...state, ...v }); setHydrated(true); });
     
  }, [key, enabled]);

  useEffect(() => {
    if (!enabled || !hydrated) return;
    const id = setTimeout(() => { drafts.set(key, state).catch(() => {}); }, 350);
